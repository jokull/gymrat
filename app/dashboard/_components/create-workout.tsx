...
export function CreateWorkout({
	...
	const displayDescription =
		description === "" && suggestedDescription ? suggestedDescription : description;
	...
	return (
		<form onSubmit={handleSubmit}>
			<CreateWorkoutFieldset
				...
				setValue={setValue}
			/>
			{mutation.error ? <p className="mt-2 text-red-500">{mutation.error.message}</p> : null}
		</form>
	);
}
...
export function CreateWorkoutFieldset({
	...
	const [localDescription, localSetDescription] = useState(defaultDescription ?? "");
	...
	return (
		<fieldset className="flex w-full flex-wrap items-end gap-4" disabled={pending}>
