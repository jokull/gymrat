...
// MIT Licensed hook from
// https://github.com/its-danny/use-lilius/blob/main/src/use-lilius.ts
...
export function useCalendar({
	...
	const viewMonth = (month: Month) => {
		setViewing(setMonth(viewing, month));
	};
	...
	const viewYear = (year: number) => {
		setViewing(setYear(viewing, year));
	};
...
export function useControlledCalendarDate({ weekStartsOn = Day.SUNDAY, value }: ControlledOptions) {
	const inRange = (date: Date, min: Date, max: Date) =>
		(isEqual(date, min) || isAfter(date, min)) && (isEqual(date, max) || isBefore(date, max));

	const selected = clearTime(value);
	const [viewing, setViewing] = useState<Date>(selected);
	...
	const isSelected = (date: Date) => isEqual(selected, date);
