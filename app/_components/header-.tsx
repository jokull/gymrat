...
export default async function Header({ page }: { page: "dashboard" | "index" | "auth" }) {
	const session = await unsealData<{ id: string; email: string } | Record<string, never>>(
				...
				{"email" in session ? (
					<>
						{page === "dashboard" ? (
							...
							<GhostLink href="/dashboard" className="flex gap-2" prefetch={false}>
								<span>Dashboard</span>
